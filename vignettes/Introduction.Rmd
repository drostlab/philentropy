...
Comparison is a fundamental method of scientific research leading to more general insights about the processes that might have caused the corresponding similarity or dissimilarity. In statistical terms comparisons between probability functions are made to infer connections, correlations, or relationships between samples.
...
__Side Note__: I am aware that some of the functions implemented in `philentropy` can be found in base R or in published CRAN packages. However, my focus was to optimize all functions implemented in `philentropy`. For this purpose all methods are implemented in C++ using the [Rcpp](http://www.rcpp.org/) API and by writing extensive unit tests I try to reduce the number of potential bugs. I really hope that some of these functions are helpful to the R community.
...
Since my first stats class I am fascinated by the potential power of determining correlation and causality in data. For me part of the `philentropy` package it some kind of playground to better understand the diversity of methods that aim to perform [correlation detection](http://www3.nd.edu/~mclark19/learn/CorrelationComparison.pdf) and [causal inference](http://ftp.cs.ucla.edu/pub/stat_ser/r350.pdf). In detail, `philentropy` will implement classical linear and non-linear correlation methods as well as causal inference methods that aim to predict relationships between (random) variables. 
