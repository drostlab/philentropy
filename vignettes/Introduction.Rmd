...
Comparison is a fundamental method of scientific research leading to more general insights about the processes causing similarity or dissimilarity. In statistical terms comparisons between probability functions are performed to infer connections, correlations, or relationships between samples.
...
__Side Note__: I am aware that some of the functions implemented in `philentropy` can be found in base R or in published CRAN packages. However, my focus was to optimize all functions implemented in `philentropy`. For this purpose all methods are implemented in C++ using the [Rcpp](http://www.rcpp.org/) API and wroteextensive unit tests aiming to reduce the number of potential bugs. I really hope that some of these functions are helpful to the R community.
